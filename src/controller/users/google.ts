...
export default async (
  ...
  try {
    ...
    const googleResponse = await axios.post(
      `https://oauth2.googleapis.com/token?client_id=${process.env.GOOGLE_CLIENT_ID}&client_secret=${process.env.GOOGLE_CLIENT_SECRET}&code=${authorizationCode}&grant_type=authorization_code&redirect_uri=https://www.bitda.ga/users/signin`
    );
